	...
	<div class="forecast">
		<div>
			<svg width="18" height="18" xmlns="http://www.w3.org/2000/svg" xml:space="preserve" viewBox="0 0 512 512">
				<title>{{ $t('weather.svg.today') }} {{ weather.temperature }}&thinsp;°F and {{ weather.description }}</title>
			...
			<span class="current" v-show="weather.temperature"><span class="visually-hidden">{{ $t('weather.current_temp') }}</span>{{ weather.temperature }}&thinsp;<abbr :title="$t('weather.degrees_f')">&deg;F</abbr></span>
			<span class="range"><span class="visually-hidden">{{ $t('weather.high_temp') }}</span>{{ weather.high }}&thinsp;&deg; <span class="low" v-show="weather.low"><span class="visually-hidden">{{ $t('weather.low_temp') }}</span>{{ weather.low }}&deg;</span></span>
		</div>
	</div>
</template>
