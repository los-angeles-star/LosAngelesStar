	...
	<div class="forecast">
		<div>
			<svg width="18" height="18" xmlns="http://www.w3.org/2000/svg" xml:space="preserve" viewBox="0 0 512 512">
				<title>{{ $t('weather.svg.today') }} {{ temperature || weather.temperature }}&thinsp;°F and {{ weather.description }}</title>
				<g v-if="weather.description = 'Clear'">
					...
					<path v-if="time == 'night'" fill="#1d1d1b" d="M350 343a128 128 0 1 1-133-211 16 16 0 0 1 20 21 95 95 0 0 0 122 122c6-2 13-1 17 4 4 4 6 11 4 16-7 19-17 35-30 48zM191 185a96 96 0 1 0 143 127 127 127 0 0 1-134-134l-9 7z"/>
			...
			<span class="current" v-show="temperature || weather.temperature"><span class="visually-hidden">{{ $t('weather.current_temp') }}</span>{{ weather.temperature || temperature }}&thinsp;<abbr :title="$t('weather.degrees_f')">&deg;F</abbr></span>
			<span class="range"><span class="visually-hidden">{{ $t('weather.high_temp') }}</span>{{ weather.high || high }}&thinsp;&deg; <span class="low" v-show="low || weather.low"><span class="visually-hidden">{{ $t('weather.low_temp') }}</span>{{ low || weather.low }}&deg;</span></span>
		</div>
	</div>
</template>

<script setup lang="ts">
import { ref, watch, onBeforeMount, onMounted, onBeforeUnmount, computed } from 'vue';
...
let timer: any = useInterval(() => { weather.getWeather(); minutesToMidnight(); }, 300000);
let today = new Date();
// get today's sunlight times for Los Angeles
let times = SunCalc.getTimes(new Date(), 34.054834228856684, -118.23919363559173);
...
function minutesToMidnight () {
	const thisMidnight = new Date();
	thisMidnight.setHours(24, 0, 0, 0);
	const minToMidnight: number = (thisMidnight.getTime() - today.getTime()) / 1000 / 60;
	midnight.value = thisMidnight;
	if (minToMidnight < 1440 && minToMidnight > 1435) {
		setSunrise();
		setSunset();
		weather.getForecast();
	}
...
function setPeriod() {
	...
	if (sunrise.value === null || sunset.value === null) { return; }
	if (now > sunrise.value.getTime() && sunset.value.getTime() >= now) {
		return time.value = 'day';
	} else {
		return time.value = 'night';
	}
}
