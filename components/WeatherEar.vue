  ...
  <div class="forecast">
    <div>
      <svg width="18" height="18" xmlns="http://www.w3.org/2000/svg" xml:space="preserve" viewBox="0 0 512 512">
        <title>{{ $t('weather.svg.today') }} {{ temperatureToFahrenheit }}&thinsp;°F and {{ weather.description }}</title>
      ...
      <span class="current" v-show="weather.temperature"><span class="visually-hidden">{{ $t('weather.current_temp') }}</span>{{ temperatureToInteger }}&thinsp;<abbr :title="$t('weather.degrees_f')">&deg;F</abbr></span>
      <span class="range"><span class="visually-hidden">{{ $t('weather.high_temp') }}</span>{{ highToInteger }}&thinsp;&deg; <span class="low" v-show="weather.low"><span class="visually-hidden">{{ $t('weather.low_temp') }}</span>{{ lowToInteger }}&deg;</span></span>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    temperatureToFahrenheit() {
      ...
      return ((1.8 * this.weather.temperature) + 32)
    },
    temperatureToInteger() {
      return Math.round( this.temperatureToFahrenheit )
    },
    lowToFahrenheit() {
      if ( !this.weather.low ) return ''
      return ((1.8 * this.weather.low) + 32)
    },
    lowToInteger() {
      return Math.round( this.lowToFahrenheit )
    },
    highToFahrenheit() {
      if ( !this.weather.high ) return ''
      return ((1.8 * this.weather.high) + 32)
    },
    highToInteger() {
      return Math.round( this.highToFahrenheit )
    },
  },
  created () {
    this.setSunrise();
